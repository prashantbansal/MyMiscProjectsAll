...
public class AdventureWorksSalesData
{
    ...
    public StringDictionary GetSalesPersonData(string salesPersonID)
    {
        ...
        const string query =
           "SELECT [FirstName] + ' ' + [LastName] AS [FullName],[Phone], [EmailAddress], [TerritoryName], [SalesQuota], [SalesYTD] FROM [AdventureWorks].[Sales].[vSalesPerson] WHERE ([SalesPersonID] = @SalesPersonID)";
