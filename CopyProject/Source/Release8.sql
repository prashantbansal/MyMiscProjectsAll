...
--#endregion
--#region [uspUpdateEssayAnswerText] 
IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[uspUpdateEssayAnswerText]')
			AND type IN (
				N'P'
				,N'PC'
				)
...
BEGIN
	DROP PROCEDURE [dbo].[uspUpdateEssayAnswerText]

	PRINT 'Dropped:[uspUpdateEssayAnswerText]'
END
GO

CREATE PROCEDURE [dbo].[uspUpdateEssayAnswerText] @pUserContentID INT
	,@pAnswerText NTEXT
	,@pIsFlagged SMALLINT = 0
	,@pTimeSpent INT
AS
...
PRINT 'Created:[uspUpdateEssayAnswerText]'
GO

--#endregion
--#region [uspSaveTexasUserAnswerText] 
IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[uspSaveTexasUserAnswerText]')
			AND type IN (
				N'P'
				,N'PC'
				)
...
BEGIN
	DROP PROCEDURE [dbo].[uspSaveTexasUserAnswerText]

	PRINT 'Dropped:[uspSaveTexasUserAnswerText]'
END
GO

CREATE PROCEDURE [dbo].[uspSaveTexasUserAnswerText] (
    @pUserContainerID INT
    ,@pUserContentId INT
    ,@pTSATableVar TexasShortAnswerTableVar readonly
    )
AS
BEGIN
    SET NOCOUNT ON;
    ...
    IF EXISTS (
            SELECT 1
            FROM UserContainer
            WHERE UserContainerID = @pUserContainerID
                AND IsCompleted <> 1
            )

    MERGE TexasUserAnswer AS T
    USING @pTSATableVar AS S
        ON T.UserContentID = @pUserContentID
            AND T.TexasQuestionID = S.TexasQuestionID
    WHEN MATCHED
        THEN
            UPDATE
            SET T.AnswerText = S.AnswerText
                ,T.Score = S.Score
                ,T.AnswerStatus = S.AnswerStatus
    WHEN NOT MATCHED BY TARGET
        THEN
            INSERT (
                UserContentID
...
PRINT 'Created:[uspSaveTexasUserAnswerText]'
GO
